...
class FakeHX711:
    ...
    def rising_edge(self) -> None:
        if not self._shifting:
            ...
            self._begin_frame()
        self._pulses += 1
        if self.slow_pulse_at is not None and self._pulses == self.slow_pulse_at:
    ...
    def read_dout(self) -> int:
        if self.stuck_low:
            return 0  # disconnected line: permanent low via the pull-down
        if self._shifting:
            if self._pulses >= 25:
...
def test_read_frame_decodes_positive_value():
    ...
    device = _make_device()
    assert device._read_frame(chip.dout, chip.sck) == 0x123456
...
def test_read_frame_decodes_zero():
    ...
    device = _make_device()
    assert device._read_frame(chip.dout, chip.sck) == 0
...
def test_read_frame_sign_extends_negative_value():
    ...
    device = _make_device()
    assert device._read_frame(chip.dout, chip.sck) == -12345
...
def test_read_frame_pulse_count_selects_gain(gain, pulses):
    ...
    assert device._read_frame(chip.dout, chip.sck) == 42
    _finalize(chip)
...
def test_read_frame_returns_none_when_stopped_while_waiting():
    ...
    assert device._read_frame(chip.dout, chip.sck) is None
...
def test_read_frame_raises_glitch_on_slow_pulse():
    ...
    with pytest.raises(_GlitchError, match="us"):
        device._read_frame(chip.dout, chip.sck)
...
async def test_consecutive_discards_log_one_warning(fake_chip, monkeypatch, caplog):
    import logging

    import glider.hal.devices.hx711 as hx711_module

    monkeypatch.setattr(hx711_module, "CONSECUTIVE_DISCARD_WARN", 2)
    # Saturated frames glitch regardless of timing. Trace: frame 1 discards
    # (bad=1), frame 2 is its dummy read, frame 3 discards (bad=2 -> WARN),
    # frame 4 is its dummy read, frame 5 caches -> deterministic end state.
    rail = 0x7FFFFF
...
async def test_read_before_first_sample_raises(fake_chip):
    device = _make_device()
    await device.initialize()
    try:
...
async def test_read_raises_when_sample_is_stale(fake_chip, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="5.0s old"):
        await device.read()
