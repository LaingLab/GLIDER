...
class RunnerShell(QWidget):
    ...
    def rehost_camera(self) -> None:
        """Re-insert the camera page into the Camera tab.

        The single CameraPanel is reparented out to the desktop Camera dock on a
        runner->desktop switch; switching back must return it to index 3 so the
        Camera tab is not left empty.
        """
        if self._stack.indexOf(self._camera) == -1:
            self._stack.insertWidget(3, self._camera)
